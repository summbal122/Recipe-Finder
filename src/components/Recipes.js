 ...
 const handleLetter = (letter)=> {
  ...
  <button 
   ...
   <div className='col-span-10 grid grid-cols-4'> 
    <p className="col-span-4 text-center text-dark-primary/60 tracking-wide">Discover delicious recipes by category or by their starting letter.</p>
    {categoryMeals?.length > 0 ? (
      categoryMeals.map((meal) => (
        <Link key={meal.idMeal} to={`/recipe/${meal.strMeal}`}>
          <CuisineCard cuisine={meal} />
        </Link>
      ))
